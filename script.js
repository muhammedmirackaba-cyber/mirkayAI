...
auth.onAuthStateChanged(user => {
    if(!user) {
        ...
        document.getElementById('user-email').innerText = user.email || "Misafir Kullanıcı";
...
// GROQ CEVAP (400 HATASI DÜZELTİLDİ)
    ...
    try {
        const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [{ role: "system", content: "Sen Mirkay AI'sın. Asil bir kurt gibi cevap ver." }, { role: "user", content: mesaj }]
            })
        });
        const data = await response.json();
        return data.choices[0].message.content;
    } catch (e) { return "Bağlantı hatası Mirkay! 🐺"; }
}
...
async function mesajGonder() {
    ...
    setTimeout(() => {
        ...
        if (yeniSayi % 25 === 0) {
            container.innerHTML += `<div class="msg" style="background:#fffbe6; font-size:12px; text-align:center; padding:10px;">🎬 Reklam: Premium al, kurt sürüsüne katıl!</div>`;
        }
        container.scrollTop = container.scrollHeight;
    }, 400);
}
...
function premiumSatinal() { window.location.href = "https://play.google.com/store/account/subscriptions"; }
...
function closeAllMenus() { document.getElementById('side-menu').classList.remove('active'); document.getElementById('profile-menu').classList.remove('active'); document.getElementById('overlay').style.display = 'none'; }
function resimYukle(i) { /* Resim yükleme kodu */ }
